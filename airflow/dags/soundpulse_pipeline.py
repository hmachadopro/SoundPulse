...
extract_deezer → glue_staging → glue_curated → data_quality_checks → notify_on_failure

TODO (phase 4) : brancher les GlueJobOperator sur de vrais jobs déployés et
implémenter les contrôles qualité (phase 5).
"""
...
def soundpulse_pipeline():
    ...
    extract_deezer() >> glue_staging >> glue_curated >> data_quality_checks()
