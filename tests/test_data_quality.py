...
Squelette : les fixtures `curated_titres` restent à brancher sur un vrai run
...
def test_validite_tempo(curated_titres):
    assert curated_titres["tempo"].between(0, 250).all()


def test_validite_energie(curated_titres):
    assert curated_titres["energie"].between(0, 1).all()
